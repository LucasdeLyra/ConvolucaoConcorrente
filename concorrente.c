...
typedef struct t_Args {
    int thread_id, NTHREADS, kernel_type;
            ...
            for (int y = 0; y < output_colors[c]->width; y++) {
                for (int i = 0; i < args->kernel->height; i++) {
                    for (int j = 0; j < args->kernel->width; j++) {
                        sum_kernel_1 += get_array_float(args->kernel)[i][j]*get_array(input_colors[c])[i+x][j+y];
                        if (args->kernel_type >= 2) { 
                            sum_kernel_2 += get_array_float(args->kernel)[j][i]*get_array(input_colors[c])[i+x][j+y];
                        }
                    }
                }

                if (args->kernel_type >= 2) { //Este if teoricamente não é necessário, mas não queria piorar mais o arredondamento nos casos em que não é necessário
